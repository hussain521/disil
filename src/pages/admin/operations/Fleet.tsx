...
export default function Fleet() {
  ...
  const columns: DataTableColumn<FleetOverviewTruck>[] = [
    {
      key: 'plate',
      header: t('admin.fleet.columns.plate'),
      render: (truck) => (
    ...
    {
      key: 'type',
      header: t('admin.fleet.columns.type'),
      render: (truck) => (
    ...
    { key: 'owner', header: t('admin.fleet.columns.owner'), render: (truck) => <span className="text-xs font-medium text-admin-text">{truck.owner?.fullName ?? '—'}</span> },
    { key: 'agent', header: t('admin.fleet.columns.agent'), render: (truck) => <span className="text-xs font-medium text-admin-text">{truck.agent?.fullName ?? '—'}</span> },
    { key: 'driver', header: t('admin.fleet.columns.driver'), render: (truck) => <span className="text-xs font-medium text-admin-text">{truck.driver?.fullName ?? '—'}</span> },
    {
      key: 'status',
      header: t('admin.fleet.columns.status'),
      render: (truck) =>
    ...
    <div className="space-y-6 animation-fade-in">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-admin-subtext">
            {overview ? t('admin.fleet.subtitle', { count: overview.total }) : t('admin.fleet.loading')}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
          ...
          >
            ...
            <span>{t('admin.fleet.advancedSearch')}</span>
          </Link>
          <Link
            to="/admin/track"
          ...
          >
            ...
            <span>{t('admin.fleet.liveMap')}</span>
          </Link>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="w-full sm:w-auto overflow-x-auto no-scrollbar">
          <FilterTabs
            tabs={[
              { value: 'approved', label: t('admin.fleet.tabs.approved') },
              { value: 'pending_review', label: t('admin.fleet.tabs.pending_review') },
              { value: 'rejected', label: t('admin.fleet.tabs.rejected') },
            ]}
      ...
      <DataTable
        ...
        loading={loading}
        emptyMessage={t('admin.fleet.noTrucks')}
